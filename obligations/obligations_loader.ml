...
let obligations_repo_path   = ".fe/obligations-repo.sexp"

let load =
  let obligations_repo_path = Relpath.of_string obligations_repo_path in
  fun (module Hg : Hg_dependency.S) repo_root ~aliases ->
    ...
    | Ok repo_declarations ->
      ...
      let try_globalspec specfile =
        match%map
          Reader.load_sexps (Abspath.to_string specfile)
            Obligations_global.Declaration.t_of_sexp
        with
        | Ok _ as ok -> ok
          ...
          | `Scaffold_exists (Ok contents) ->
            Or_error.try_with (fun () ->
              Sexp.of_string_conv_exn ("(" ^ contents ^ ")")
                [%of_sexp: Obligations_global.Declaration.t list])
      in
