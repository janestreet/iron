...
let of_matches ~file_names ~context diamond matches =
  ...
  let enqueue_segment (segment : Segment.t) =
    ...
    | None ->
      ...
      current_rev_common := [];
      current_segment := Some (Segment.prepend pre_common segment
                              , Segment.is_shown segment);
    | Some (seg1, last_of_seg1_is_shown) ->
      let seg2 = segment in
      let context_between = List.rev !current_rev_common in
      current_rev_common := [];
      (* either we merge them in which case they end up in the current segment or we
         don't in which case we distribute the context accordingly, push the current into
         the finished segments and replace the current segment *)
...
let of_files ?(verbose=false) ?(force_should_split_files_in_hunks=false)
      ~rev_names ~context ~contents:diamond () =
